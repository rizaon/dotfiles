...
;; ----------------------------------------------------------------
;; haryadi, this is where i define the coloring !!!!!
;; markdown-inline-code-face : blue
;; markdown-blockquote-face : red
;; markdown-list-face : yellow
;; markdown-header-face  : blue
;; markdown-link-face : purple
;; markdown-url-face : green
;; markdown-reference-face : green
;; italic-face: yellow
;; bold face: green
...
(defcustom my-face-red' font-lock-comment-face "")
(defcustom my-face-red' font-lock-comment-delimiter-face "")
(defcustom my-face-green' font-lock-doc-face "")
(defcustom my-face-green' font-lock-string-face "")
(defcustom my-face-cyan' font-lock-keyword-face "")
(defcustom my-face-darkblue' font-lock-builtin-face "")
(defcustom my-face-darkblue' font-lock-function-name-face "")
(defcustom my-face-yellow' font-lock-variable-name-face "")
(defcustom my-face-green' font-lock-type-face "")
(defcustom my-face-pink' font-lock-constant-face "")
(defcustom my-face-xb' font-lock-preprocessor-face "")
(defcustom my-face-xc' font-lock-negation-char-face "")
(defcustom my-face-xd' font-lock-warning-face "")
; ----------------------------------------------------------------
...
(defcustom markdown-italic-face my-face-red
  "Italic text."
  :group 'markdown-faces
  :type '(face))

;;(defcustom markdown-bold-face 'font-lock-type-face
(defcustom markdown-bold-face my-face-green
...
; ----------------------------------------------------------------
; haryadi, add five and six spaces for pre
; pre4
(defconst markdown-regex-pre
  "^    \\([a-zA-Z0-9/#%@\"?.[(*-]\\).*$"
  "Regular expression for matching preformatted text sections")
...
;   ( ( | [a-z] | [A-Z] | [0-9]  ).*$"
;DEFAULT:  "^     \\(\(\\|[a-z]\\|[A-Z]\\|[0-9]\\).*$"
(defconst markdown-regex-pre5
  "^     \\([a-zA-Z0-9/#%@\"?.[(*-]\\).*$"
  "Hsg, Regular expression for matching preformatted text sections")
; pre6
;(defconst markdown-regex-pre6
;  "^      \\(\(\\|[a-z]\\|[A-Z]\\|[0-9]\\).*$"
;  "Hsg, Regular expression for matching preformatted text sections")
; pre6all
(defconst markdown-regex-pre6all
  "^      .*$"
  "Hsg, Regular expression for matching preformatted text sections")
; ----------------------------------------------------------------
...
; ----------------------------------------------------------------
; haryadi : disable '_' as bold, you could add new char after the '*'
...
; haryadi: add my own bold with this model +abac+
(defconst markdown-regex-my-bold
  "\\(^\\|[^\\]\\)\\(\\([+]\\)\\([^ \\]\\3\\|[^ ]\\(.\\|\n\\)*?[^\\ ]\\3\\)\\)"
  "Regular expression for matching italic text")

; haryadi: add my own italic with this model =abc=
(defconst markdown-regex-my-italic
  "\\(^\\|[^\\]\\)\\(\\([=]\\)\\([^ \\]\\3\\|[^ ]\\(.\\|\n\\)*?[^\\ ]\\3\\)\\)"
  "Regular expression for matching italic text")
...
(defconst markdown-mode-font-lock-keywords-basic
  (list
   ...
   ;; ----------------------------------------------------------------
   ;; haryadi, add pre5, pre6
   (cons markdown-regex-pre  my-face-yellow)
   (cons markdown-regex-pre5 my-face-green)
   ;;(cons markdown-regex-pre6all my-face-pink)
   (cons markdown-regex-pre6all my-face-red)
   ...
   ;; ----------------------------------------------------------------
   ;; haryadi, change header to cyan
   ;;(cons markdown-regex-header-setext markdown-header-face)
   ;;(cons markdown-regex-header-atx markdown-header-face)
   (cons markdown-regex-header-setext my-face-cyan)
   (cons markdown-regex-header-atx my-face-cyan)
   ;; ----------------------------------------------------------------
   ...
   ;; ----------------------------------------------------------------
   ;; haryadi, change line to cyan
   ;; (cons markdown-regex-hr markdown-header-face)
   (cons markdown-regex-hr my-face-cyan)
   ;; ----------------------------------------------------------------
   ...
   ;; ----------------------------------------------------------------
   ;; haryadi, add my own regex-my-italic and bold
   (cons markdown-regex-my-italic '(2 my-face-pink))
   (cons markdown-regex-my-bold '(2 my-face-green))
   ;; ----------------------------------------------------------------
...
If Transient Mark mode is on and a region is active, it is made italic."
  ...
  (if markdown-italic-underscore
      ; haryadi, disable coloring for _
